...
/*
 * This file has been modified to be used in a proprietary RSS reader and is only being used for personal use.
 * The original file can be found at https://github.com/openssl/openssl/blob/master/demos/guide/tls-client-block.c
 * For the purposes of this program the main function has been converted to a callable function
 */
...
int XML_Get (int argc, char *argv[]){
    ...
    int ipv6 = 0;
    std::stringstream ss;
    std::string output, ctitle, cimage, cdesc, cimg_start, ccombined;
    std::ofstream comicf;
    ...
    while (SSL_read_ex(ssl, buf, sizeof(buf), &readbytes)) {
        ...
        */
        ...
        ss.write((const char*)buf, (std::streamsize)readbytes);  // Copy the page content into a stringstream to later convert to a string
//        fwrite(buf, 1, readbytes, ss);
    }

    output = ss.str();  // Convert the stringstream to a string
    ...
    cdesc  = output.substr(output.find("title=\""),  (output.find(" alt=") - (output.find("title=\""))));   // Find the Description of the comic
    cimage = cimage.append(cdesc);
    cimage = cimage.append(">");

    ccombined = ctitle.append(cimage);

    std::cout << std::endl << ctitle;

    comicf.open("~/comic.html");
    comicf << ccombined;
    comicf.close();
...
int main () {
    ...
    strncpy(get_args[1], "xkcd.com", 10);
